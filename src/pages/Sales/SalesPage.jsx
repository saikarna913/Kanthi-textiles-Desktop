...
// ── Category groups — values MUST match CATEGORY_CANONICAL in excelParser.js ─
const CATEGORY_GROUPS = [
  {
    label: 'Kalamkari',
    options: ['KALAMKARI SAREES', 'KALAMKARI UNSTITCHED', 'KALAMKARI DUPATTAS'],
  },
  {
    label: 'Readymades',
    options: [
      'READYMADES',
      "READYMADES - MEN'S",
      "READYMADES - WOMEN'S TOPS/KURTHIS",
    ],
  },
  {
    label: 'Other Textiles',
    options: [
      'SAREES', 'DRESS SETS', 'BAGS', 'BEDSHEETS', 'TOWELS',
      'HOME ACCESSORIES', 'LEISURE WARE', 'CARPETS', 'POCHAMPALLI MATS', 'HANKIES',
    ],
  },
];
...
const TX_COLUMNS = [
  ...
  { key:'invoice_no',   header:'Invoice', w:120,
    render: v => <span style={{ fontFamily:'monospace', fontSize:11, color:'var(--text-muted)' }}>{v||'—'}</span> },
  { key:'customer_name',header:'Customer',w:160 },
  { key:'product_name', header:'Product', w:200 },
  { key:'category',     header:'Category',w:160,
    render: v => <Badge variant="accent" size="xs">{v}</Badge> },
  { key:'quantity',     header:'Qty',     w:60,
    render: v => <span style={{ fontFamily:'monospace' }}>{fmtNum(v)}</span> },
  { key:'unit_price',   header:'Rate',    w:90,
    render: v => <span style={{ fontFamily:'monospace', color:'var(--text-primary)' }}>{fmtCur(v)}</span> },
  { key:'total_amount', header:'Amount',  w:110,
    render: v => <span style={{ fontFamily:'monospace', fontWeight:700, color:'var(--accent)' }}>{fmtCur(v)}</span> },
  { key:'profit',       header:'Profit',  w:100,
    render: v => <span style={{ fontFamily:'monospace', fontWeight:600, color:v>=0?'var(--success)':'var(--danger)' }}>{fmtCur(v)}</span> },
  { key:'payment_mode', header:'Payment', w:100,
    render: v => <Badge variant="info" size="xs">{v}</Badge> },
...
const MB_COLUMNS = [
  {
    ...
    render: v => {
      if (!v) return '—';
      // Parse "YYYY-MM-DD" by splitting on '-' to avoid any Date constructor
      // UTC-offset shifting the month (the same bug that affected the parser).
      const parts = String(v).split('-');
      if (parts.length < 2) return v;
      const year  = parseInt(parts[0], 10);
      const month = parseInt(parts[1], 10) - 1; // 0-indexed for Date.UTC
      if (isNaN(year) || isNaN(month)) return v;
      // Use UTC constructor + timeZone:'UTC' so toLocaleDateString never shifts
      return new Date(Date.UTC(year, month, 1))
        .toLocaleDateString('en-IN', { month:'long', year:'numeric', timeZone:'UTC' });
    },
  },
  { key:'category',     header:'Category',         w:220,
    render: v => v ? <Badge variant="gold" size="xs">{v}</Badge>
                   : <span style={{ color:'var(--text-muted)' }}>—</span> },
  { key:'product_name', header:'Item / Stock Item', w:360 },
  { key:'quantity',     header:'Units Sold',        w:150,
...
// ── Shared table ──────────────────────────────────────────────────────────────
function DataTable({ columns, data, loading, selectedIds, onToggleSelect, onToggleAll,
                     onDelete, sortBy, sortDir, onSort, page, totalPages, total, limit, onPageChange }) {
  ...
  return (
    <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16,
            ...
            <tr style={{ borderBottom:'1px solid var(--border)' }}>
              ...
              {columns.map(col => (
                <th key={col.key} onClick={() => onSort(col.key)}
                  style={{ textAlign:'left', padding:'10px 12px', color:'var(--text-muted)',
            ...
            {loading
              ...
              : data.map(row => (
                  <tr key={row.id}
                    ...
                    {columns.map(col => (
                      <td key={col.key} style={{ padding:'9px 12px', color:'var(--text-primary)',
                                                 maxWidth:col.w, overflow:'hidden',
                    ...
                    <td style={{ padding:'9px 12px' }}>
                      <button onClick={() => onDelete(row.id)}
                        style={{ background:'var(--danger-bg)', border:'none', borderRadius:6,
                                 cursor:'pointer', color:'var(--danger)', padding:'3px 7px', fontSize:10 }}>
                        <Trash2 size={11}/>
                      </button>
                    </td>
                  </tr>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                    padding:'9px 14px', borderTop:'1px solid var(--border)', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <button onClick={() => onPageChange(p => Math.max(1,p-1))} disabled={page===1}
            style={{ padding:'4px 8px', background:'var(--bg-hover)', border:'1px solid var(--border)',
                     borderRadius:6, cursor:'pointer', color:'var(--text-secondary)',
                     opacity:page===1?.4:1, display:'flex', alignItems:'center' }}>
            <ChevronLeft size={13}/>
          </button>
          ...
          <button onClick={() => onPageChange(p => Math.min(totalPages,p+1))} disabled={page>=totalPages}
            style={{ padding:'4px 8px', background:'var(--bg-hover)', border:'1px solid var(--border)',
                     borderRadius:6, cursor:'pointer', color:'var(--text-secondary)',
                     opacity:page>=totalPages?.4:1, display:'flex', alignItems:'center' }}>
...
function TransactionTab() {
  ...
  const handleDelete   = async id => { if (!window.confirm('Delete this record?')) return; await window.electron.db.deleteSale(id); toast.success('Deleted'); load(); };

  const handleDeleteSelected = async () => {
    if (!selectedIds.size || !window.confirm(`Delete ${selectedIds.size} selected rows?`)) return;
  ...
  const handleDeleteFiltered = async () => {
    if (!window.confirm(`Delete ALL rows matching current filters (${total.toLocaleString()} records)? This cannot be undone.`)) return;
    ...
    toast.dismiss('clr');
    if (r?.success) { toast.success(`Deleted ${r.deleted} rows`); setSelectedIds(new Set()); setPage(1); load(); }
    else toast.error(r?.error||'Failed to delete filtered rows');
  };
  const handleDeleteAll = async () => {
    if (!window.confirm(`Delete ALL ${total.toLocaleString()} sales transaction records? This is irreversible.`)) return;
    toast.loading('Deleting all sales…', { id:'clrall' });
    const r = await window.electron.db.deleteSalesByFilter({ salesType:'sales' });
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ padding:'12px 24px', flexShrink:0 }}>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', marginBottom:8 }}>
          ...
          <CategorySelect value={category} onChange={e=>{ setCategory(e.target.value); setPage(1); }}/>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          {selectedIds.size>0 && (
            <Button variant="danger" size="sm" onClick={handleDeleteSelected}>
              <Trash2 size={12} style={{ marginRight:4 }}/>Delete {selectedIds.size} selected
            </Button>
          )}
          {hasFilter && total>0 && (
            <Button variant="danger" size="sm" onClick={handleDeleteFiltered}>
              <Trash2 size={12} style={{ marginRight:4 }}/>Delete filtered ({total.toLocaleString()})
            </Button>
          )}
          {total>0 && (
            <Button variant="danger" size="sm" onClick={handleDeleteAll}>
              <Trash2 size={12} style={{ marginRight:4 }}/>Delete all sales
            </Button>
          )}
          <span style={{ fontSize:11, color:'var(--text-muted)', marginLeft:4 }}>{total.toLocaleString()} transactions</span>
        </div>
      </div>
      <div style={{ flex:1, overflow:'hidden', padding:'0 24px 16px' }}>
        <DataTable columns={TX_COLUMNS} data={data} loading={loading}
...
function MonthlySalesTab() {
  ...
  const handleDeleteFiltered = async () => {
    ...
    if (!window.confirm(`Delete ALL rows matching filters (${desc}) — ${total.toLocaleString()} records? This cannot be undone.`)) return;
    toast.loading('Deleting filtered rows…', { id:'clr' });
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm(`Delete ALL ${total.toLocaleString()} monthly sales records? This is irreversible.`)) return;
  ...
  const hasFilter    = !!(search||category||month||year);

  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ margin:'0 24px 10px', background:'var(--gold-bg)', border:'1px solid rgba(245,158,11,.25)',
                    borderRadius:10, padding:'9px 14px', fontSize:12, flexShrink:0 }}>
        <span style={{ color:'var(--gold)', fontWeight:700 }}>Monthly Sales Format</span>
        <span style={{ color:'var(--text-secondary)', marginLeft:8 }}>
          S.NO / STOCK ITEMS / TOTAL — the <b>TOTAL</b> column shows <b>units sold</b> that month (not rupees).
          Filter by month and year to see a specific period.
        </span>
      </div>
      <div style={{ padding:'0 24px 10px', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          {total>0 && (
            <Button variant="danger" size="sm" onClick={handleDeleteAll}>
              <Trash2 size={12} style={{ marginRight:4 }}/>Delete all monthly
            </Button>
          )}
          <span style={{ fontSize:11, color:'var(--text-muted)' }}>{total.toLocaleString()} line items</span>
          {totalUnits>0 && (
            <span style={{ fontSize:11, color:'var(--gold)', fontWeight:700, fontFamily:'monospace' }}>
              · {totalUnits.toLocaleString()} units (this page)
            </span>
          )}
        </div>
      </div>
      <div style={{ flex:1, overflow:'hidden', padding:'0 24px 16px' }}>
        <DataTable columns={MB_COLUMNS} data={data} loading={loading}
          selectedIds={selectedIds} onToggleSelect={toggleSelect} onToggleAll={toggleAll}
          onDelete={handleDelete} sortBy={sortBy} sortDir={sortDir} onSort={handleSort}
          page={page} totalPages={totalPages} total={total} limit={limit} onPageChange={setPage}/>
      </div>
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'20px 24px 0', flexShrink:0 }}>
        <PageHeader
          ...
          icon={ShoppingCart} iconColor="var(--info)"
        />
        <div style={{ display:'flex', borderBottom:'1px solid var(--border)', gap:0 }}>
          <TabBtn active={activeTab==='transactions'} onClick={()=>setActiveTab('transactions')} color="var(--accent)">
            <ShoppingCart size={14}/>Sales Transactions
          </TabBtn>
          <TabBtn active={activeTab==='monthly'} onClick={()=>setActiveTab('monthly')} color="var(--gold)">
            <Layers size={14}/>Monthly Sales Data
          </TabBtn>
        </div>
      </div>
